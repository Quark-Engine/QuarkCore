...
int main() {
    qc::InitWindow(1280, 720, "QuarkCore VK Sandbox", qc::RendererType::Vulkan);
    ...
    qc::Texture2D vulkan = qc::LoadTexture("resources/vulkan.png");
    ...
    while (!qc::WindowShouldClose()) {
        ...
        qc::DrawTextureEx(
            vulkan,
            qc::Vec2{
                static_cast<float>(qc::GetScreenWidth()) - vulkan.width * vulkanScale,
                vulkanTopMargin
            },
            0.0f,
            vulkanScale,
